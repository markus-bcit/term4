...
resource "aws_dynamodb_table" "terraform_locks" {
  ...
  billing_mode = "PAY_PER_REQUEST" # best case we would set our use and go the pre paid route since its cheaper
  hash_key     = "LockID" # specifying the key 
  attribute {
    name = "LockID" # name of the key, since the table needs a unique id 
    type = "S" # string
  }
}
