...
locals {
  ...
  project_name      = "acit4640_wk06"
  availability_zone = "us-west-2a"
  ssh_key_name      = "acit4640_wk06"
}
...
# be used in the  in AWS CLI Operations
resource "local_file" "aws_setup" {
  ...
  SSH_KEY="${aws_key_pair.ssh_key_pair.key_name}"
  DB01_ID="${aws_instance.ec2_set["db01"].id}"
  %{for instance in aws_instance.ec2_set~}
${substr(upper(instance.tags["name"]), 14, -1)}="${instance.id}"
  %{endfor~}
