...
PROXY_CERT    = './server-certs/proxy.crt'
PROXY_PRIVATE = './server-certs/proxy.key'
...
def deal_with_client(ssock_cli, server_name):
    ...
    if request:
        ...
        pattern = re.compile (b"If-None-Match", re.IGNORECASE)
        request = pattern.sub(b"*************", request)
        pattern = re.compile (b"If-Modified-Since", re.IGNORECASE)
        request = pattern.sub(b"*****************", request)
        print(request)
